...
<div class="profile-content-box p-6 rounded-2xl bg-white dark:bg-slate-850 border border-slate-200 dark:border-slate-800 shadow-elevation-1 flex flex-col md:flex-row md:items-center justify-between gap-6 overflow-hidden">
    <div class="flex items-center space-x-5">
        <div class="relative shrink-0">
            <img src="{{ $user->profile?->avatar_url ?? 'https://ui-avatars.com/api/?name=' . urlencode($user->name) }}" 
                 alt="{{ $user->name }}" 
                 class="w-20 h-20 rounded-2xl object-cover border-2 border-brand-500 shadow-md adaptive-icon">
        ...
        <div class="space-y-1">
            <div class="flex items-center space-x-2">
                <h2 class="text-2xl font-bold text-slate-900 dark:text-white tracking-tight stat-value">{{ $user->name }}</h2>
                <span class="px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider bg-brand-500/10 text-brand-600 dark:text-brand-400 border border-brand-500/20">
                    {{ is_object($user->role) && method_exists($user->role, 'label') ? $user->role->label() : ($user->role ?? 'User') }}
                </span>
            </div>
            <p class="text-xs text-slate-500 dark:text-slate-400 font-medium subtext">{{ $user->email }}</p>
            <div class="flex items-center space-x-3 text-[11px] text-slate-400 subtext">
                <span>Bergabung {{ $user->created_at?->format('M Y') ?? now()->format('M Y') }}</span>
                <span>•</span>
